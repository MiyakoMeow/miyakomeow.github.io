...
- BMS数据：参考项目目录下类似路径，如 `D:\Codes\bms-table-mirror\tables\[stellabms.xyz] Satellite\data.json`
- Header配置：参考 `static/bms/table/satellite-skill-analyzer-3rd-preview/header.json`
...
for song in songs:
    for item in data:
        if song in item.get('title', ''):
            print(item.get('title') + ': level=' + item.get('level') + ', md5=' + item.get('md5'))
```
...
先读取现有的header.json，了解当前course结构，然后添加新course：
